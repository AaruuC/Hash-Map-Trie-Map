...
public class HashMap<K, V> extends BaseAbstractMap<K, V> {
    ...
    public V put(K key, V value) {
        ...
        if (entry != null) {
            ...
            Entry<K, V> newEntry = new Entry<K, V>(key, value, table[bucket]);
        ...
        if (size() == MAXIMUM_CAPACITY) {
            ...
            for (int i = 0; i < table.length; i++) {
                ...
                while (entry != null) {
                    ...
                    int hash = hash(entry.getKey().hashCode(), newHashTable.length);
                    entry.next = newHashTable[hash];
                    newHashTable[hash] = entry;
                    entry = next;
                }
            }
    ...
    public V remove(Object key) {
        ...
        if (table[bucket].getKey().equals(key)) {
            Entry<K, V> removed = table[bucket];
        ...
        while (curr != null && curr.getKey() != null && !curr.getKey().equals(key)) {
            curr = curr.next;
            prev = curr;
        }
        if (curr != null) {
    ...
    public boolean containsValue(Object value) {
        ...
        while (iter.hasNext()) {
            ...
            if (next.getValue() != null && next.getValue().equals(value)) {
                return true;
    ...
    protected Iterator<Map.Entry<K, V>> entryIterator() {
        ...
        return new LazyIterator<Map.Entry<K, V>>(size());
    }
    
    class LazyIterator<E> implements Iterator<Map.Entry<K, V>> {
        Entry<K, V> curr;
        int size;
        int total;
        int bucket;
        
        LazyIterator(int size) {
            this.size = size;
            this.curr = table[0];
        }
